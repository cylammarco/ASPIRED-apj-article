...
def make_bias_master(folder_path,
    ...
    bias_list_temp = glob.glob(os.path.join(folder_path, "*.bz2"))
    bias_list = []
    bias_rc = []

    for bias_path in bias_list_temp:
        bias_list.append(bias_path)
        temp = fits.open(os.path.join(bias_path))
    ...
    if save_bias:
        ...
        if 'npy' in save_bias_format:
            if save_folder_path is None:
                np.save(os.path.join(folder_path, save_bias_name + '.npy'),
                        bias_master)
                np.savetxt(os.path.join(folder_path,
        ...
        if ('fits' or 'fit') in save_bias_format:
            ...
            if save_folder_path is None:
                ...
                bias_fits = fits.PrimaryHDU(bias_fits)
                # Save file to disk
