...
quality. \textcolor{red}{\bf Some moderate one-time effort in modifying the
configuration will allow for repeatable science-ready reduced spectral data}
from a \textcolor{red}{\bf reduced image}. More fine-tuning and extra (pre-)processing can
extend the reduction to systems with more complex setups. We would like to
stress that, although only a few parameters are needed to be updated,
it does not mean that little time is needed to get those parameters correct
and suitable for generalisation to the instrument, because of, for example,
instrument stability. We compare some
...
unfortunately, \textcolor{red}{\bf the National Optical Astronomy Observatory 
...
and high data rates. However, \textcolor{red}{\bf such tools are not always available.
...
spectral reduction (but \textcolor{red}{\bf only handle sky subtraction if a sky frame is provided}). In the \textsc{Astropy}
...
graduate-level astronomers. \textcolor{red}{\bf Similar to \textsc{iraf},
...
\textcolor{red}{\bf by the transient science research group at Tel Aviv University
for a proprietary spectral data reduction pipeline of the Las Cumbres Observatory FLOYDS instrument. It is also known to be used}
...
also \url{https://github.com/cylammarco/aspired-example}}, and an excerpt of
...
software that is as modular and portable as possible, \textcolor{red}{\bf such that
it is operable on \textsc{Linux}, \textsc{Mac} and \textsc{Windows} systems (while software for Astronomy and Astrophysics are
strongly leaning towards Unix systems, \textsc{Windows} has the highest
...
finding), as well as complications due to blended lines. \textcolor{red}{\bf 
...
\textcolor{red}{\bf \textsc{rascal} does not require templates and is designed to
...
\textcolor{red}{\bf not available in \citet{2018ApOpt..57.6876S}}.
...
human-readable. In view of that, \textsc{ASPIRED} provides \textcolor{red}{\bf input
...
\textcolor{red}{\bf \textsc{ASPIRED} only intends to provide image reduction methods
...
\textcolor{red}{\bf In systems with more complex optical design, for example,
instruments having multiple non-parallel spectra illuminated on the
...
extinction correction, \textcolor{red}{\bf telluric absorption correction, and
...
\begin{figure}
    ...
    sub-spectra \textcolor{red}{\bf with a resampling factor of 1.0}. The \textcolor{red}{\bf light grey, dark grey and black} boxes are three example
    ...
    cross-correlated function for the \textcolor{red}{\bf light grey} sub-spectrum relative
    to the \textcolor{red}{\bf dark grey} sub-spectrum (plotted in alternating \textcolor{red}{\bf light and dark grey} dashes), and the one
    for the \textcolor{red}{\bf black} sub-spectrum relative to the \textcolor{red}{\bf dark grey} sub-spectrum
    (plotted in alternating \textcolor{red}{\bf dark grey and black} dashes).}
    \label{fig:trace}
\end{figure}

\textcolor{red}{\bf 
In the cross-correlation process, ASPIRED supports the up-sampling of the data. 
Because more than one target could be incident on the slit, we do not attempt
to fit any profile at this stage. Hence, the shifts found are always quantized to an integer pixel level.
However, the polynomial fitted through the shifts as a function of the dispersion
direction is a continuous function. The final precision depends on the up-sampling rate.
If no up-sampling is performed and the spectrum is tilted by one pixel across the
entire range of dispersion, for example, this step will not be able to get the tilt correctly.
...
optimal extraction algorithms can maximize the SNR. They work by down-weighting
...
\begin{figure}
    ...
    for clarity). \textit{Bottom}: The independently normalized electron
...
\begin{figure}
    ...
    \textcolor{red}{\bf Extraction using \texttt{Horne86} and \texttt{Marsh89} algorithm produce
...
\textcolor{red}{\bf range of } 100 to 30\,000 \AA\ 
...
wavelength-calibrated standard spectrum with the literature values to map the electron count to flux as a function of wavelength. \textcolor{red}{\bf All
...
\textcolor{red}{\bf It is optional for the sensitivity function to be} derived from
the continuum of the standard spectrum. This \textcolor{red}{\bf removes} absorption
...
The continuum is \textcolor{red}{\textbf{found using the} \textsc{specutils}'s
\textbf{\texttt{fits\_continuum}}}. This can remove any outlying random noise and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{red}{\bf \section{Validation}
\label{sec:examples}
}
...
\textcolor{red}{\bf The purpose of the comparison made here is to demonstrate that
...
\textcolor{red}{\bf \subsection{Example Data Reduction Results}
}
...
produce spectral products at a quality comparable to \textcolor{red}{\bf some
...
\end{tabular}
    \caption{List of the eight targets used in Figures~\ref{fig:reduction_compared} \& \ref{fig:use_cases} \textcolor{red}{\bf in alphabetical order of the telescope names}. See body text for references.}
    \label{tab:summary}
\end{table*}
...
\textcolor{red}{\bf \subsection{Repeatability}}
\textcolor{red}{\bf In order to demonstrate reproducibility and repeatability,
we opt to calibrate the standard star BD\,33+2642 against Hiltner\,102 on
eight nights for which \textit{LT}/SPRAT observed both on the same night during
...
and at the end of the night. The data used are not taken immediately
...
\begin{figure}
    ...
    \caption{\textcolor{red}{\bf Fractional residual in the calibrated flux of BD\,33+2642 using
...
\textcolor{red}{
\bf All the reduction scripts for the spectra used in this article can be found at
...
that is tagged by version number starting from \texttt{v0.4}. \textcolor{red}{\bf This work is most accurately
