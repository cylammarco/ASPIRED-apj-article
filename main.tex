...
spectroscopic observations. The Automated SpectroPhotometric
REDuction~(\textsc{ASPIRED}) toolkit is a \textsc{Python}-based spectral data
reduction toolkit. It is designed to be a general toolkit with high flexibility
for users to refine and optimise their data reduction routines for the
...
quality. Minor configuration changes can enable reliable and repeatable
publication quality data reduction of spectral observations from a simple
instrument. More fine-tuning and extra (pre-)processing can extend the
reduction to systems with more complex setups. We compare some example spectra
reduced with \textsc{ASPIRED} to published data processed with
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We use the SPRAT spectrograph mounted on the \textit{Liverpool Telescope}~(\textit{LT}) as our
``first-light'' instrument for development~\citep{2014SPIE.9147E..8HP}. \textsc{ASPIRED} is also currently used for the quicklook reduction of data obtained by the MISTRAL spectrograph
...
Spectroscopy~\citep[GTC/OSIRIS;][]{2000SPIE.4008..623C}, and \textit{Telescopio
...
compare then with published results.
...
top of \textsc{ASPIRED} without needing to adapt the code. They work seamlessly
for a few reasons: first, \textsc{Python} is an interpreter, it can
execute in run time. It is hence straightforward to run it \textsc{Python}-server to
...
sets of arc lines that follow a linear relation. \textsc{rascal} extends to
...
correct matches from both the red and blue ends of the spectrum than the
...
\textsc{rascal} and uses their native plotting capability. Since the flux calibration has to be performed
...
\texttt{SpectrumOneD} object which gets passed from a \texttt{TwoDSpec} object. In Astronomy, the handling of observational
...
\begin{figure}
    ...
    counts across the two slices, the much higher baseline on the right is
    indicating a low signal-to-background ratio. The vertical black dashed
    ...
    clarity. On the right, it illustrates how an extraction over a non-rectified
    two-dimensional spectrum can lead to an increased sky background level. The
    wing region on the left of the centre of the trace at pixel 161 is tilting
    upwards in the electron counts.}
...
have to be done outside of \textsc{ASPIRED} beforehand. We compare the reduction against
...
from strong fringing effects. This is not handled by \textsc{ASPIRED} natively as well,
...
exposures\footnote{\url{https://telescope.livjm.ac.uk/cgi-bin/lt_search}}~\citep{2020MNRAS.499.2959H}. The \textit{VLT}/FORS2 data
...
which is built on top of \textsc{STARLINK}. All of the spectra are resampled to match
...
in Figure~\ref{fig:use_cases}, the \textit{GTC}/OSIRIS shows on a blue large amplitude
...
\begin{figure*}
    ...
    of absorption features. The \textit{TNG}/DOLORES spectra were of two targets where the
    tracing was done simultaneously and then the extraction done~(red hue)
    sequentially. The \textit{WHT}/ISIS and ACAM (green hue and yellow) are extracting a
...
>> python setup.py
\end{verbatim}
. We distribute through \textsc{conda} starting from \texttt{v0.4.9}.
...
that is tagged by the version from \texttt{v0.4}.
