...
spectral reduction (but only handle sky subtraction if a sky frame is provided).
...
graduate-level astronomers. Similar to \textcolor{red}{\uline{\textsc{iraf}}},
...
it is operable on \textsc{Linux}, \textsc{Mac} and \textsc{Windows}
...
and other utilities to facilitate the development. It uses \textsc{git}\footnote{\url{https://git-scm.com}},
issue and bug tracking, high-level project management, and automation with \textsc{Github Actions} upon each \texttt{commit} for:
...
sub-pixel peak finding), as well as complications due to blended lines.
...
\textcolor{red}{\uline{\textsc{rascal}}} does not require templates and is designed to
...
In systems with more complex optical design, for example,
...
\begin{figure}
    ...
    sub-spectra with a resampling factor of 1.0. The light grey, dark
    grey and black boxes are three example sub-spectra. Bottom left: the
    three sub-spectra are summed in the dispersion direction to generate
    the three profiles that are cross-correlated to compute the shift.
    Bottom right: the cross-correlated function for the light grey
    sub-spectrum relative to the dark grey sub-spectrum (plotted in
    alternating light and dark grey dashes), and the one for the black
    sub-spectrum relative to the dark grey sub-spectrum (plotted in
    alternating dark grey and black dashes).}
    \label{fig:trace}
\end{figure}
...
The final precision depends on the up-sampling rate. If no up-sampling is
...
The resampling is performed with \textcolor{red}{\uline{\textsc{spectresc}}}. It is possible to supply
a set of pre-computed polynomials to perform the rectification, which can
...
\begin{figure}
    ...
    Extraction using \texttt{Horne86} and \texttt{Marsh89} algorithm produce
...
available in \textsc{iraf}, on the Issac Newton Group of Telescopes webpage,
and on the ESO webpage -- \textit{Optical and UV Spectrophotometric Standard Stars}
are available on \textsc{ASPIRED}\footnote{\url{https://cylammarco.github.io/SpectroscopicStandards/}}. We call the set of data a \textit{library},
...
that is tagged by version number starting from \texttt{v0.4}. This work is most
accurately reflecting the software release of the \texttt{v0.5}-series.
\textcolor{red}{\uline{We encourage users to reduce example spectra available in these repositories, using their own pipelines, for comparison.}}
