...
y_tmp = ndimage.zoom(
    np.array(red.spectrum_list[0].trace), zoom=upsample_factor
)
...
img_tmp = ndimage.zoom(img_tmp, zoom=1.0 / upsample_factor)
...
ax1.imshow(
    np.log10(red.img),
    vmin=np.nanpercentile(np.log10(red.img), 15),
    vmax=np.nanpercentile(np.log10(red.img), 97.5),
    origin="lower",
    aspect="auto",
)
ax2.imshow(
